...
function SearchingChallenge(strArr: string[]): number {
  const numRows: number = strArr.length;
  const numCols: number = strArr[0].length;
  ...
  const varFiltersCg: number[] = matrix.flat().filter(cell => cell === 0);
